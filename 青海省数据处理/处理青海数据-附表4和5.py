...
def readstationinfo(sheet_index):
    data = np.array(sheet_index)
...
def configuration(dict_station, sheet_index, vehtype, year1, month1, day1,
    ...
    dic_result = dealdata(dict_station, filepath, vehtype, start_time, end_time, tollway, origin_col)
    print("完成第二阶段：处理数据")
...
if __name__ == "__main__":
    ...
    '''
    # 获取收费站和路段对应字典
    filename_open = r"F:\科研项目\高速公路\青海省资料\青海省实车测试的表格-20200325\Data\0-辅助表.xlsx"
    ...
    configs = [[2, 1, 2019, 12, 1, 2019, 12, 31, 1, 4,  0, '.xlsx', '.xls'],
    ...
    for config in configs:
        configuration(dict_station2road, *config)
        print('完成一次迭代')
